...
You may separately authorize automatic sharing for an exact future source/project scope on the final manual Submit screen or later from Settings. When a valid hosted challenge is available, the final screen selects the combined submit-and-enable choice by default; it displays the cadence and cap, links to the full versioned recurring authorization, retention, ownership certification, and exact scope, and lets you uncheck the choice before submitting. If recurring authority is already configured, the same row remains checked and locked so the manual share cannot silently alter that authority; Settings remains the place to review, pause, or revoke it. The local recurring mode remains off until the manual share receives a hosted receipt; only then may the client consume the receipt-issued enrollment grant and create recurring authority. If the scope or hosted terms changed, automatic enrollment stops and the receipt page asks you to review them again. The recurring path is limited to Claude Code and Codex, whose append-only inputs have strict parsing and content-bound mutation checks; other sources remain manual-share only. Raw custom redaction strings, allowlist values, usernames, and local session IDs are not sent as enrollment metadata.
