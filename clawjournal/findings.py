...
def allowlist_add_by_hash(
    ...
    *,
    ...
    # Deliberately no auto-upload pause here: the findings allowlist is not
    # part of the recurring egress profile hash, so pausing guarded nothing —
    # resume()'s profile check passed unchanged and the runner re-derives
    # every per-finding tier decision from this table at package time. The
    # allowlist decision itself is the explicit, audited user review.
...
def allowlist_remove(
    ...
    # No auto-upload pause: removing an entry only reverts findings toward
    # the stricter open/redact states, and the findings allowlist is not an
    # egress-profile-hash input (see allowlist_add_by_hash).
