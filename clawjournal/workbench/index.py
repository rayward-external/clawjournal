...
CREATE TABLE IF NOT EXISTS auto_upload_enrollment (
    ...
    enrolled_scope_entries_json     TEXT NOT NULL,
...
def _migrate_exact_scope_pairs(conn: sqlite3.Connection) -> None:
    """Advance v9 -> v10 with the exact recurring source/project snapshot.

    Existing enrollments were authorized and filtered as the full cross
    product of their stored source and project lists. Preserve that exact
    behavior during migration; a later user-approved reauthorization can
    replace it with the observed-pair scope generated by current clients.
    """

    version_row = conn.execute("PRAGMA user_version").fetchone()
    version = version_row[0] if version_row else 0
    if version >= EXACT_SCOPE_PAIRS_SCHEMA_VERSION:
        return
    ...
    try:
        try:
            conn.execute(
                ...
                "ADD COLUMN enrolled_scope_entries_json TEXT"
            )
        except sqlite3.OperationalError as exc:
            if "duplicate column" not in str(exc):
                raise
        ...
        row = conn.execute(
            "SELECT enrolled_sources_json, enrolled_projects_json, "
            "enrolled_scope_entries_json "
            "FROM auto_upload_enrollment WHERE singleton_id = 1"
        ).fetchone()
        if row is not None and row["enrolled_scope_entries_json"] is None:
            sources = _canonical_auto_upload_strings(
                json.loads(row["enrolled_sources_json"]),
                field="enrolled_sources",
            )
            projects = _canonical_auto_upload_strings(
                json.loads(row["enrolled_projects_json"]),
                field="enrolled_projects",
            )
            entries = [
                [source, project]
            ...
            conn.execute(
                "UPDATE auto_upload_enrollment "
                "SET enrolled_scope_entries_json = ? WHERE singleton_id = 1",
...
_AUTO_UPLOAD_SCOPE_ENTRIES_FIELD = "enrolled_scope_entries_json"
...
def _canonical_auto_upload_scope_entries(
    values: Iterable[Sequence[str]],
    *,
    field: str = "enrolled_scope_entries",
) -> list[tuple[str, str]]:
    if isinstance(values, (str, bytes)):
        raise ValueError(f"{field} must be a collection of source/project pairs")
    ...
    for value in values:
        if (
            ...
            raise ValueError(f"{field} must contain only source/project pairs")
        ...
        if (
            ...
            raise ValueError(
                f"{field} must contain only non-empty source/project strings"
            )
        normalized.add((source.strip(), project.strip()))
    if not normalized:
        raise ValueError(f"{field} must not be empty")
    return sorted(normalized)
...
def _decode_auto_upload_enrollment(row: sqlite3.Row) -> dict[str, Any]:
    ...
    if _AUTO_UPLOAD_SCOPE_ENTRIES_FIELD not in enrollment:
        # Read-only status/preview may inspect a v9 database before any
        # writable command has run the v10 migration. Preserve the old scope
        # semantics until open_index() persists the same cross-product snapshot.
        decoded_entries = [
            [source, project]
            for source in enrollment["enrolled_sources"]
            for project in enrollment["enrolled_projects"]
        ]
    else:
        raw_entries = enrollment.pop(_AUTO_UPLOAD_SCOPE_ENTRIES_FIELD)
        try:
            decoded_entries = json.loads(raw_entries)
        except (TypeError, json.JSONDecodeError) as exc:
            raise ValueError(
                f"corrupt automatic-upload field: {_AUTO_UPLOAD_SCOPE_ENTRIES_FIELD}"
            ) from exc
    enrollment["enrolled_scope_entries"] = _canonical_auto_upload_scope_entries(
        decoded_entries
    )
...
def save_auto_upload_enrollment(
    ...
    *,
    ...
    enrolled_scope_entries: Iterable[Sequence[str]] | None = None,
    ...
    canonical_sources = _canonical_auto_upload_strings(
        enrolled_sources, field="enrolled_sources"
    )
    canonical_projects = _canonical_auto_upload_strings(
        enrolled_projects, field="enrolled_projects"
    )
    canonical_entries = _canonical_auto_upload_scope_entries(
        enrolled_scope_entries
        if enrolled_scope_entries is not None
        else (
            (source, project)
    ...
    if any(
        source not in canonical_sources or project not in canonical_projects
        for source, project in canonical_entries
    ):
        raise ValueError(
            "enrolled_scope_entries must be contained by enrolled sources/projects"
        )
    ...
    payload: dict[str, Any] = {
        ...
        "enrolled_scope_entries_json": json.dumps(
            canonical_entries,
            separators=(",", ":"),
        ),
...
def update_auto_upload_enrollment(
    ...
    **changes: Any,
    ...
    for field, value in changes.items():
        if field == "enrolled_scope_entries":
            stored_changes[_AUTO_UPLOAD_SCOPE_ENTRIES_FIELD] = json.dumps(
                _canonical_auto_upload_scope_entries(value),
                separators=(",", ":"),
            )
        elif field in _AUTO_UPLOAD_LIST_FIELDS:
    ...
    *,
    ...
    for row in rows:
        ...
        if row["project"] not in enrolled_projects:
            ...
            continue
        if (row["source"], row["project"]) not in enrolled_scope_entries:
            exclude(row, "scope_pair_excluded")
            continue
        if scope_blockers:
